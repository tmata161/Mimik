...
#include"../../lib/USB/Human_Interface_Device_Class/script_injection.h"
//------------------------------------
...
void handleOK(fileExplorerObject* exobj){
    ...
    if(exobj->TFC>0){
       if((exobj->files[exobj->fP[exobj->fSP]].fattrib) == AM_DIR && exobj->files[exobj->fP[exobj->fSP]].fname!=0){
        ...
        // if it is a file
        if((exobj->files[exobj->fP[exobj->fSP]].fattrib) == AM_ARC &&  exobj->files[exobj->fP[exobj->fSP]].fname!=0){
            // check if it is a text file and within memory bounds

            //deallocate screen buffer to make heap space
